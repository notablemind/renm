...
let rec down =
  ...
  if (node.children != []
      && (node.id == data.root || Set.String.has(expanded, node.id))) {
...
let nextChildPosition =
  ...
  if (node.id == data.root
      || node.children != []
      && Set.String.has(expanded, node.id)) {
    (node.id, 0);
  } else {
    {
      let%Opt parent = Map.String.get(data.nodes, node.parent);
...
let orderIds = (nodes, root, ids) => {
  let rec loop = (id, collected) => {
    ...
    ->List.reduce(collected, (collected, child) => loop(child, collected));
  };
  ...
  Map.String.map(nodes, node =>
    {
      ...node,
      Data.Node.children:
        node.Data.Node.children->List.keep(Map.String.has(nodes)),
    }
  );
