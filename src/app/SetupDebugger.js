...
let formatObject = c => c === undefined
? "None"
: ["object", { "object": c}];
...
var variantCustomFormatter = function (data,recordVariant){
  if(recordVariant === "::"){
    return [
      ...
      return ["ol", olStyle, ...data.map(function (cur) { return formatObject(cur) })]
  }
...
var formatter = {
  ...
  body: function (x) {
      ...
      if ( (recordLabels = x[Symbol.for('BsRecord')]) !== undefined
        ...
        return formatObject(x[1])
      }
      else if(isOCamlExceptionOrExtension(x)){
        return ["ol", olStyle, ... x.slice(1).map(cur => formatObject(cur) )]
      }
